...
import { BaseEndpointTypes } from '../endpoint/grade-improvement'
...
export const httpTransport = new HttpTransport<HttpTransportTypes>({
  ...
  parseResponse: (params, response) => {
    ...
    if (!response.data) {
      return params.map((param) => {
        return {
          params: param,
          response: {
            errorMessage: `The data provider didn't return any value for ${param.data}/${param.tx}`,
            statusCode: 502,
          },
        }
      })
    ...
    return params.map((param) => {
      const result = response.data.data.inferenceResult
